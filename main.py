...
def is_price_sane(symbol, price):
...
def _score_v61(closes, highs, lows, vols, i, category="EQUITIES_INDICES"):
    ...
    if ma200 is None or sma20 is None or atr_v is None: return None
    ...
    return {
        "action": action, "score": round(score, 2),
        "reasons": reasons, "confidence": confidence,
        "levels": levels,
        "metrics": {
            "atr_pct": round(atr_pct, 2), "adx": round(adx_v, 1),
            "rsi": round(rsi_val, 1) if rsi_val is not None else None,
            "macd": macd, "ma200": round(ma200, 2), "sma20": round(sma20, 2),
...
def _simulate_v61(closes, highs, lows, vols, symbol):
    ...
    while i < len(closes) - 3:
        ...
        for day in range(1, max_hold + 1):
            ...
            if direction == 1:
                if day_high > highest: highest = day_high
                ...
                if day_low < lowest: lowest = day_low
        ...
        trades.append({
            "symbol": symbol, "entry_idx": i, "exit_idx": i + exit_day,
            "outcome": "win" if pnl > 0 else "loss" if pnl < 0 else "breakeven",
            "pnl_pct": round(pnl, 2), "exit_reason": exit_reason,
            "duration": exit_day, "action": sig["action"],
        })
        i = i + exit_day + 1
    return trades
...
async def fetch_yahoo_chart_with_retry(session, yahoo_symbol, max_retries=None):
    """Yahoo chart with retry on 429/5xx."""
    if max_retries is None: max_retries = BACKTEST_MAX_RETRIES
    url = f"https://query1.finance.yahoo.com/v8/finance/chart/{yahoo_symbol}"
    for attempt in range(max_retries + 1):
        try:
            async with session.get(
                url, params={"interval": "1d", "range": "1y"},
                headers=HEADERS, timeout=aiohttp.ClientTimeout(total=BACKTEST_TIMEOUT),
            ) as resp:
                if resp.status == 200:
                    return await resp.json()
                if resp.status in (429, 500, 502, 503, 504):
                    wait = 0.5 * (2 ** attempt)
                    logger.warning(f"[{yahoo_symbol}] HTTP {resp.status} retry {attempt+1}/{max_retries}")
                    await asyncio.sleep(wait)
                    continue
                return None
        except (asyncio.TimeoutError, aiohttp.ClientError) as e:
            wait = 0.5 * (2 ** attempt)
            logger.warning(f"[{yahoo_symbol}] {type(e).__name__} retry {attempt+1}/{max_retries}")
            await asyncio.sleep(wait)
            continue
        except Exception as e:
            logger.warning(f"[{yahoo_symbol}] unexpected: {e}")
            return None
    return None


async def _fetch_yahoo_cached(session, sym, yahoo, semaphore):
    """v6.1.0: Yahoo fetch with 15min cache + semaphore + jitter."""
    now = datetime.now()
...
async def fetch_one(session, key):
    ...
    try:
        async with session.get(url, headers=HEADERS, timeout=aiohttp.ClientTimeout(total=8)) as resp:
            ...
            if price <= 0: return key, None
            if not is_price_sane(key, price): return key, None
            change = ((price - prev) / prev * 100) if prev else 0
            return key, {
                "symbol": key, "price": round(price, 2),
...
async def get_all_prices():
    ...
    if not results:
        raise HTTPException(503, "No data fetched")
    _cache["data"] = results
...
async def _run_backtest(refresh: bool, now: datetime):
    """Inner backtest logic."""
    all_results = []
    by_symbol = {}
    fetch_stats = {
        "requested": 0, "fetched": 0, "short_data": 0,
        "fetch_failed": 0, "no_trades": 0, "failed_symbols": [],
    }
    async with aiohttp.ClientSession() as session:
        ...
        for item in gathered:
            ...
            try:
                ...
                if len(closes) < 200:
                    ...
                    logger.info(f"[{sym}] only {len(closes)} days")
                    continue
                trades = _simulate_v61(closes, highs, lows, vols, sym)
                if not trades:
                    fetch_stats["no_trades"] += 1
                    logger.info(f"[{sym}] no trades")
                    continue
                wins = sum(1 for t in trades if t["outcome"] == "win")
                losses = sum(1 for t in trades if t["outcome"] == "loss")
                timeouts = sum(1 for t in trades if "time" in t["exit_reason"])
                total = len(trades)
                equity = [10000.0]
                for t in trades:
                ...
                all_results.append({
                    "symbol": sym, "trades": trades, "total": total,
